...
# Try to install BLAS/LAPACK libraries (may have different names on different OS versions)
echo "Installing BLAS/LAPACK libraries..."
sudo apt install -y liblapack-dev libblas-dev || echo "Note: Some BLAS/LAPACK packages not available, continuing..."
sudo apt install -y libatlas-base-dev 2>/dev/null || sudo apt install -y libopenblas-dev 2>/dev/null || echo "Note: ATLAS not available, using system BLAS"
